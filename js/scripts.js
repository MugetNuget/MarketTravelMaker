...
function Cal_B_Prof(precios){
    ...
    keys=Object.keys(precios);
    resultados={};
    for(let key of keys){
        ppc=Object.values(precios[key]);
        ppck=Object.keys(precios[key]);
        ...
        min=Math.min(...ppc);
        gain=max-min;
        profit=(max-min)*100/min;
        ruta="From "+String(ppck[ppc.indexOf(min)])+ " to "+String(ppck[ppc.indexOf(max)]);
        resultados[key]={};
...
Listas={ CLOTH:[],METALBAR:[],FIBER:[],HIDE:[],LEATHER:[],ORE:[],PLANKS:[],STONEBLOCK:[],WOOD:[] }
